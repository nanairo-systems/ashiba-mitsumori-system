...
| ETCアラート（タブ） | `/accounting/etc`（アラートタブ） | ✅ 実装済み |
| ETCアラート（専用ページ） | `/accounting/etc/alerts` | ✅ 実装済み |
| 社員マスター | `/accounting/masters`（社員タブ） | ✅ 実装済み |
| 高速道路ICマスター | `/accounting/etc`（アラートタブ内で管理） | ✅ 実装済み |
...
Employee（社員マスター）→ EtcDriver（紐付け）
HighwayIC（高速道路ICマスター）→ ETCアラート検知
```
...
|-----------|-----|------|
| id | TEXT（UUID） | 主キー |
| name | TEXT | IC名（UNIQUE） |
| roadName | TEXT? | 路線名 |
| area | TEXT | エリア区分（デフォルト: normal） |
| note | TEXT? | 備考 |
...
| departmentId | TEXT? | Department への FK |
| storeId | TEXT? | Store への FK |
| employeeId | TEXT? | Employee への FK |
| note | TEXT? | 備考 |
| isActive | BOOLEAN | 有効フラグ |
| createdAt / updatedAt | TIMESTAMP | 自動設定 |
...
#### 社員管理（EmployeeMasterList）
- 会社選択で絞込（部門経由）
- テーブル表示: 氏名・部門・店舗・役職・電話・メール・状態
- 操作: 新規登録・編集・無効化・有効化・削除（DEVELOPER限定）
- ETCドライバー管理の「社員から選択」で参照される
- API: `GET/POST /api/accounting/employees`, `PATCH/DELETE /api/accounting/employees/[id]`

### 4-3. ETC管理（`/accounting/etc`）

#### タブ構成
- 利用明細 / 月別集計 / 車両別月次 / アラート / 車両管理 / ドライバー管理 / カード管理 / CSVインポート
...
#### アラートタブ（EtcAlerts）
- ICマスター状態パネル（登録数表示 + 初期データ投入/追加登録ボタン）
- サマリーカード（7種別ごとの件数、クリックでフィルター）
- アラート詳細リスト（車両・ドライバー・金額・日時表示）
- 「アラート詳細ページ」へのリンク

#### アラート専用ページ（`/accounting/etc/alerts`・EtcAlertPage）
- 日付範囲フィルター（From/To）
- 種別フィルター・重要度フィルター・テキスト検索
- サマリーカード8枚（全件 + 7種別、クリックで絞込）
- アラート一覧（展開で詳細表示: カード番号・車両・ドライバー・金額・IC・利用種別）
- CSV出力機能
- ICマスター状態パネル
...
| 種別 | コード | 検知条件 |
|------|--------|---------|
| 未登録IC | unknown_ic | ICマスターに未登録の行先 |
| 高額利用 | high_amount | 閾値（デフォルト5,000円）以上 |
| 休日/深夜 | unusual_time | 土日・祝日・22時〜6時の利用 |
| 初行先 | first_ic | 過去3ヶ月で初めてのIC利用 |
| 高頻度 | high_frequency | 同カード1日5回以上 |
| 月間急増 | monthly_spike | 前月比150%以上 |
| 平均超過 | above_avg | 個人過去平均の250%以上 |

#### ドライバー管理タブ（EtcDriverManager）
- ドライバー一覧（名前・部門・店舗・担当カード/車両）
- 新規登録（名前・部門・店舗選択 / 社員マスターから選択）
- 編集・無効化
...
### GET/POST `/api/accounting/employees`
```
GET Response: Employee[]（department・store を include）
POST Request: { name, departmentId?, storeId?, phone?, email?, position?, note? }
```

### PATCH/DELETE `/api/accounting/employees/[id]`
```
PATCH Request: { name?, departmentId?, storeId?, phone?, email?, position?, note?, isActive? }
DELETE: DEVELOPER権限必須。紐付くEtcDriverがある場合は削除不可。
```

### GET/POST/PUT `/api/accounting/etc/highway-ics`
```
GET Response: HighwayIC[]
POST Request: { name, roadName?, area?, note? }
PUT: 東海地方約100IC初期データシード
```

### GET `/api/accounting/etc/alerts`
```
Query: ?from=YYYY-MM-DD&to=YYYY-MM-DD&months=1&threshold=5000
Response: { alerts[], summary, icMasterCount, period }
アラート7種類: unknown_ic, high_amount, unusual_time, first_ic, high_frequency, monthly_spike, above_avg
```
...
### 高速道路ICマスター（シードAPI）
- 東海地方約100IC（東名, 新東名, 名神, 東名阪, 伊勢湾岸, 伊勢道, 中央道, 東海北陸, 名二環, 名古屋高速, 東海環状, 知多半島道路）
- PUT `/api/accounting/etc/highway-ics` でシード

### DDLファイル
- `supabase-etc-ddl.sql`: ETCテーブル作成SQL
  - v1: 基本テーブル（EtcVehicle, EtcDriver, EtcCard, EtcRecord）
  - v2: 部門・店舗・配車履歴（departmentId/storeId + EtcDriverAssignment）
  - v3: 社員マスター（Employee + EtcDriver.employeeId）
  - v4: 高速道路ICマスター（HighwayIC）
- v2〜v4 DDL は 2026-03-10 にSupabase DBへ適用済み
