...
export async function POST(req: Request) {
  ...
  const rows = XLSX.utils.sheet_to_json<EtcRow>(ws, { defval: null })

  // 既存カードマップ（カード番号 → id）
  const existingCards = await prisma.etcCard.findMany({ select: { id: true, cardNumber: true } })
  ...
  for (const row of rows) {
    ...
    const amount = Number(row["金額（税込）"] ?? row["金額"] ?? 0)
  ...
  const existingRecords = await prisma.etcRecord.findMany({
  ...
  await prisma.etcRecord.createMany({ data: newRecords })
