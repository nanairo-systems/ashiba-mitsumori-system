/**
 * [COMPONENT] 請求書印刷レイアウト - InvoicePrint
 *
 * A4サイズの請求書。自社情報・請求先情報・現場名・請求明細・合計金額を記載。
 */
"use client"
  ...
  return (
    <>
      ...
      <div className="print:hidden fixed top-0 left-0 right-0 z-50 bg-white border-b shadow-sm px-6 py-3 flex items-center gap-3">
        ...
        <span className="text-sm text-slate-500 flex-1 truncate">請求書 {invoice.invoiceNumber}</span>
        <Button onClick={() => window.print()} className="gap-2"><Printer className="w-4 h-4" />印刷する（PDF保存）</Button>
      </div>

      {/* 印刷エリア */}
      <div className="pt-[60px] print:pt-0">
        <div className="relative mx-auto bg-white w-[210mm] min-h-[297mm] px-[15mm] py-[12mm] shadow-xl print:shadow-none print:w-full print:min-h-0" style={{ fontFamily: "'Noto Sans JP', sans-serif" }}>
          {/* タイトル */}
          ...
          <div className="flex justify-between mb-8">
            <div className="w-[48%]">
              <p className="text-lg font-bold border-b-2 border-black pb-1 mb-2">
                {invoice.contract.company.name}　御中
              </p>
              {invoice.contract.contact && (
                <p className="text-sm text-slate-600">{invoice.contract.contact.name} 様</p>
              )}
            </div>
            <div className="w-[40%] text-right">
              <p className="text-sm text-slate-500 mb-1">
                請求日: {format(new Date(invoice.invoiceDate), "yyyy年MM月dd日")}
              </p>
              {invoice.invoiceNumber && (
                <p className="text-xs text-slate-400 mb-1">請求番号: {invoice.invoiceNumber}</p>
              )}
              {invoice.dueDate && (
                <p className="text-xs text-slate-500 mb-2">
                  お支払期限: {format(new Date(invoice.dueDate), "yyyy年MM月dd日")}
                </p>
              )}
            ...
            <table className="w-full text-sm">
              <tbody>
                <tr className="border-b border-slate-100">
                  <td className="py-1.5 text-slate-600">請求金額（税抜）</td>
                  <td className="py-1.5 text-right font-mono">¥{formatCurrency(invoice.amount)}</td>
                </tr>
                <tr className="border-b border-slate-100">
                  <td className="py-1.5 text-slate-600">消費税</td>
                  <td className="py-1.5 text-right font-mono">¥{formatCurrency(invoice.taxAmount)}</td>
                </tr>
                <tr className="border-t-2 border-black">
                  <td className="py-2 font-bold">合計（税込）</td>
                  <td className="py-2 text-right font-mono font-bold text-lg">¥{formatCurrency(invoice.totalAmount)}</td>
                </tr>
              </tbody>
            </table>
          </div>
  ...
  return (
    <>
      <tr className="bg-slate-50">
        <td colSpan={5} className="py-1 px-2 text-xs font-semibold text-slate-600 border-b border-slate-100">{section.name}</td>
      </tr>
...
function GroupRows({ group }: { group: InvoiceGroup }) {
  return (
    <>
      <tr>
        <td colSpan={5} className="py-0.5 px-2 pl-4 text-xs text-slate-500 border-b border-slate-50">{group.name}</td>
      </tr>
