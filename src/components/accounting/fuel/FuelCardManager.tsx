...
export function FuelCardManager({ initialCards, vehicles, drivers }: Props) {
  ...
  async function handleAdd() {
    ...
    setLoading(true)
    try {
      const res = await fetch("/api/accounting/fuel/cards", {
        method: "POST",
      ...
      if (!res.ok) { toast.error(data.error ?? "登録に失敗しました"); return }
  ...
  async function handleEdit(id: string) {
    setLoading(true)
    try {
      const res = await fetch(`/api/accounting/fuel/cards/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      setCards((prev) => prev.map((c) => c.id === id ? { ...c, vehicleId: editForm.vehicleId || null, driverId: editForm.driverId || null, note: editForm.note || null, vehicle, driver } : c))
      setEditId(null)
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-base font-semibold text-slate-800">ガソリンカード一覧</h2>
        <button
        ...
        <div className="bg-white rounded-xl border border-orange-200 p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" value={form.vehicleId} onChange={(e) => setForm({ ...form, vehicleId: e.target.value })}>
                <option value="">未設定</option>
                {vehicles.map((v) => <option key={v.id} value={v.id}>{v.nickname ?? v.plateNumber}</option>)}
              </select>
            </div>
            <div>
              <label className="text-xs text-slate-500 mb-1 block">紐づけドライバー</label>
              <select className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" value={form.driverId} onChange={(e) => setForm({ ...form, driverId: e.target.value })}>
                <option value="">未設定</option>
            ...
            <div>
              ...
              <input className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} />
            </div>
          </div>
          <div className="flex gap-2">
            <button onClick={handleAdd} disabled={loading} className="flex items-center gap-1.5 px-4 py-2 bg-orange-600 text-white rounded-lg text-sm font-medium hover:bg-orange-700 disabled:opacity-50 transition-colors">
            ...
            {editId === c.id ? (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs text-slate-500 mb-1 block">紐づけ車両</label>
                    <select className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" value={editForm.vehicleId} onChange={(e) => setEditForm({ ...editForm, vehicleId: e.target.value })}>
                      <option value="">未設定</option>
                      {vehicles.map((v) => <option key={v.id} value={v.id}>{v.nickname ?? v.plateNumber}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="text-xs text-slate-500 mb-1 block">紐づけドライバー</label>
                    <select className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" value={editForm.driverId} onChange={(e) => setEditForm({ ...editForm, driverId: e.target.value })}>
                      <option value="">未設定</option>
                      {drivers.map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="text-xs text-slate-500 mb-1 block">備考</label>
                    <input className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" value={editForm.note} onChange={(e) => setEditForm({ ...editForm, note: e.target.value })} />
                  </div>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => handleEdit(c.id)} disabled={loading} className="flex items-center gap-1 px-3 py-1.5 bg-orange-600 text-white rounded-lg text-xs font-medium hover:bg-orange-700 disabled:opacity-50">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      {!c.vehicle && !c.driver && <span className="text-xs text-slate-400">未設定</span>}
                    </div>
