...
export function EtcMonthlySummary() {
  const [data, setData] = useState<MonthSummary[]>([])
  const [loading, setLoading] = useState(true)
  const [expandedMonth, setExpandedMonth] = useState<string | null>(null)

  useEffect(() => {
    fetch("/api/accounting/etc/monthly-summary")
      .then((res) => res.json())
      .then((d) => {
        setData(d)
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 h-7 bg-slate-100 rounded-md overflow-hidden relative">
                      <div
                        className="h-full bg-emerald-400 group-hover:bg-emerald-500 rounded-md transition-all duration-300"
                ...
                return (
                  <>
                    <tr
                      ...
                      onClick={() =>
                        setExpandedMonth(isExpanded ? null : month.yearMonth)
                      }
                    >
