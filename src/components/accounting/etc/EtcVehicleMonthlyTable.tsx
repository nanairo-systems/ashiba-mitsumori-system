...
export function EtcVehicleMonthlyTable() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/accounting/etc/vehicle-monthly?${params}`)
  ...
  // 各車両の行合計
  const vehicleRowTotals = vehicles.map((v) =>
    months.reduce((sum, ym) => sum + (v.monthly[ym]?.amount ?? 0), 0)
  )
        ...
        <table className="w-full text-sm border-collapse min-w-[600px]">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200">
              ...
              <th className="text-left px-3 py-3 text-xs font-semibold text-slate-600 min-w-[90px]">
                担当者
              </th>
              <th className="text-left px-3 py-3 text-xs font-semibold text-slate-600 min-w-[140px] hidden lg:table-cell">
                部門
              </th>
              <th className="text-left px-3 py-3 text-xs font-semibold text-slate-600 min-w-[80px] hidden lg:table-cell border-r border-slate-200">
                店舗
              </th>
              {months.map((ym) => (
                <th
                  key={ym}
                  className="text-right px-3 py-3 text-xs font-semibold text-slate-600 min-w-[100px]"
                >
                  {formatMonth(ym)}
                </th>
              ))}
          ...
          <tbody className="divide-y divide-slate-100">
            {vehicles.map((v, idx) => (
              <tr key={v.vehicleKey} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-3 py-2.5 text-xs text-slate-400 hidden lg:table-cell border-r border-slate-100">
                  {v.storeName || "—"}
                </td>
                {months.map((ym) => {
                ...
                <td className="px-4 py-2.5 text-right font-bold text-slate-800 bg-slate-50 border-l border-slate-200 tabular-nums">
                  ¥{vehicleRowTotals[idx].toLocaleString()}
                </td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr className="bg-slate-100 border-t-2 border-slate-300">
              <td className="px-4 py-3 font-bold text-slate-700 sticky left-0 bg-slate-100 z-10 border-r border-slate-200">
                月合計
              </td>
              <td className="px-3 py-3 bg-slate-100" />
              <td className="px-3 py-3 bg-slate-100 hidden lg:table-cell" />
              <td className="px-3 py-3 bg-slate-100 hidden lg:table-cell border-r border-slate-200" />
              {months.map((ym) => (
                <td key={ym} className="px-3 py-3 text-right font-bold text-slate-700 tabular-nums">
                  ¥{(monthlyTotals[ym] ?? 0).toLocaleString()}
                </td>
              ))}
    ...
    <div className="bg-white rounded-xl border border-slate-200 p-4">
      <div className="flex flex-wrap items-end gap-3">
        ...
        <div>
          ...
          >
            {monthOptions.filter((m) => m.value >= fromMonth).map((m) => (
              <option key={m.value} value={m.value}>{m.label}</option>
            ))}
          </select>
        </div>
